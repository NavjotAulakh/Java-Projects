...
public class FileImpl extends UnicastRemoteObject implements FileInterface
    ...
    public String register(String username, String password) throws IOException {
        ...
        if (userPat.matcher(username).matches() & username.length() > 3) {
            ...
            if (checkUsername(username) == 1) {
                ...
                writeUserPassToFile(username, password);
    ...
    public void writeUserPassToFile(String username, String password) throws IOException {
        ...
        fileOut.append(username + "#" + password + "#" + username); //Appends the string parameters into the file
     ...
     * @param fileName A file's name, as listed in the server's file directory. Thankfully, the names are captured perfectly for the client.
     * @throws RemoteException As with all remote object methods, it throws a special type of exception when the registry is not available.
            ...
            try
            {
                ...
                while ((line = fileIn.readLine()) != null) 
				{
                    ...
                    if (username.equals(temp[0])) 
					{
						...
						String group;
						StringTokenizer st = new StringTokenizer(temp[2], "|", false);
    ...
    public void addUserToGroup(String username, String group)
    {
        try
        {
            ...
            while ((line = fileIn.readLine()) != null) {
                ...
                if (username.equals(temp[0])) {
                    //Checks if password matches the password part of the line for matches username
                    newLine = temp[0] + "#" + temp[1] + "#" + temp[2] + '|'+ group;
			...
			File tempFile = new File("loginInfo.txt");
            tempFile.delete();
            BufferedWriter fileOut = new BufferedWriter(new FileWriter("loginInfo.txt", true));
			inputStr = inputBuffer.toString();

            fileOut.write(inputStr, 0, inputStr.length());

            fileOut.close(); //Closes the file
        }
        catch (Exception e)
        {
            System.out.println ("Error writing to loginInfo.txt Ex:" + e);
        }
    }

    public void createGroup(String msg, String groupName)
    {
       try
       {
           new File("Files\\"+ groupName + "\\").mkdirs();
           BufferedWriter fileOut = new BufferedWriter(new FileWriter("Files\\"+ groupName + "\\" + "groupInfo.txt", true));
    ...
    public String getGroupMsg(String groupName)
    {
        try
        {
            BufferedReader fileIn = new BufferedReader(new FileReader("Files\\"+ groupName + "\\" + "groupInfo.txt"));
            String line;
            line = fileIn.readLine();
            fileIn.close();
            return line;
        ...
        {
            System.out.println("Error getting group msg:" + e);
            return null;
        }
    }
